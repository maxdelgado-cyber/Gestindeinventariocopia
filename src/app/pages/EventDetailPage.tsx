          ...
          <CardContent className="pt-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-lg p-4">
                <h4 className="font-semibold text-sm text-purple-600 mb-2">Valor Total</h4>
                <p className="text-3xl font-bold text-purple-900">${evento.valorTotal.toLocaleString('es-CL')}</p>
              </div>

              <div className={`border rounded-lg p-4 ${
                evento.estadoPago === 'Pagado' ? 'bg-gradient-to-br from-green-50 to-green-100 border-green-200' :
                evento.estadoPago === 'Abonado' ? 'bg-gradient-to-br from-yellow-50 to-yellow-100 border-yellow-200' :
                'bg-gradient-to-br from-red-50 to-red-100 border-red-200'
              }`}>
                <h4 className={`font-semibold text-sm mb-2 ${
                  evento.estadoPago === 'Pagado' ? 'text-green-600' :
                  evento.estadoPago === 'Abonado' ? 'text-yellow-600' :
                  'text-red-600'
                }`}>Estado de Pago</h4>
                <p className={`text-2xl font-bold ${
                  evento.estadoPago === 'Pagado' ? 'text-green-700' :
                  evento.estadoPago === 'Abonado' ? 'text-yellow-700' :
                  'text-red-700'
                }`}>{evento.estadoPago || 'Pendiente de pago'}</p>
              </div>

              <div className="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-lg p-4">
                <h4 className="font-semibold text-sm text-blue-600 mb-2">Monto Pagado</h4>
                <p className="text-3xl font-bold text-blue-900">${(evento.montoPagado || 0).toLocaleString('es-CL')}</p>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm text-purple-700">
                  {Math.round(((evento.montoPagado || 0) / evento.valorTotal) * 100)}% completado
                </span>
              </div>
              ...
              <div className="w-full bg-gray-200 rounded-full h-4 overflow-hidden mb-3">
                <div
                  className={`h-4 rounded-full transition-all ${
                    evento.estadoPago === 'Pagado' ? 'bg-green-500' :
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="font-bold text-purple-900">${(evento.montoPagado || 0).toLocaleString('es-CL')}</p>
                </div>
                {evento.valorTotal > (evento.montoPagado || 0) && (
                  <div className="text-right">
                    <p className="text-xs text-purple-600">Saldo Pendiente</p>
                    <p className="font-bold text-orange-600">${(evento.valorTotal - (evento.montoPagado || 0)).toLocaleString('es-CL')}</p>
                  </div>
                )}
              </div>
            </div>
